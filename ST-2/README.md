...
## ゲーム概要
  - 習作シューティング ゲーム. 全 19 ステージでエンディングです<br>
 - ラスボスは非常に硬いです **(Lv1弾30000発以上の耐久力!)**. 敵をやっつけたら現れる 'L' アイテムをどんどん取って, レベル アップしてください!
  - キーボード
    - **SWAD, UMHK, カーソルキー** ... 自機の移動
    - **スペース, Z, V** ... ショット
    - **F1, F3** ... 決定, ポーズ, ゲーム再開, スキップに使用します
    - **F2, F4** ... スキップ, キャンセルに使用します
  - ジョイスティック
    - MZ-1X03 方式と AM7J 方式を自動検出・判別します
    - MZ-1X03 は, センターがズレてるかもしれません
    - MZ-1X03 の互換ジョイスティックは, プロトコルをあまり守ってないものも多そうなので, <span style="color: red; ">自動判別しないで強制的に MZ-1X03 とみなすモード</span> を用意しました<br>
      設定は, ゲームモード選択画面にあります<br>
...
|--------|----------|----------------------|---------------------------|--------------------------|--------|
|EASY    |LV50      |普通                  |3機スタート,<br>得点で増加 |なし      |LV50からスタート<br>エンディングなし |
...
|むぼう  |LV1       |ノーコメント          |1機スタート,<br>得点で増加 |なし      |いきなり最終面<br>ボスの硬さを知るがいい! |
...
## EmuZ-700 + 互換 JP-ROM でしか動作確認してませんが, 皆様のご協力で実機(日本版)での確認が進んでます
  ...
  - 速度は遅くなったり速くなったりしてませんか? (問題なし: 動画を見てる限り, 問題なさそう)
  - 三重和音の音色は綺麗に出てます? (問題なし: 動画を見てる限り, 音色もほぼ同じ)
  - NWK ロゴは正しく表示されてます? (問題なし: 2 回のバージョンアップを経て正しく表示するようになりました)<br>
  - ジョイスティックは反応しますか? (問題なし: 左右同時押しでポーズがかかるので, 1.01 では TOWNS PAD対応は廃止)
...
* 2025.10.06 1.15
  - MZ-1X03 の感度調整がおかしいバグ修正 (最初からあったバグらしい)
  - 操作性の向上<br>
  ...
  - バイナリ提供で mzt 形式の他 wav ファイルも用意しました (PC から直接 MZ-700 に送っても, テープに録音してロードしても OK)
* 2026.01.03 1.16
  - MZ-1X03 で勝手にボタンが押されてしまうことがあるバグ修正 (最初からあったバグらしい. EmuZ-700 2025.11.23 版にて発覚)
