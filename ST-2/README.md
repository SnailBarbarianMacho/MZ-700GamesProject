...
Sorry it's not in English. Please read it by machine translation etc.
...
## ゲーム概要
  ...
  - 非常に硬いラスボスを倒すため, 'L' アイテムを取って, どんどんレベル アップしてください!<br>
    ラスボスは, 初期弾換算で30000発以上の耐久力があります!
  - 敵をなぎ倒す快感を追求し, 連射度は高めにしました
  - アトラクト モード時は右キーで次の画面に移ります
  - ジョイスティック(AM7J のみ)対応. SELECT/RUN ボタンが誤動作の報告があったので非対応としました.
  - 制作期間は, 2021.07～10.<br>
...
## EmuZ-700 + 互換 JP-ROM でしか動作確認してませんが, 皆様のご協力で実機(日本版)での確認が進んでます
  ...
  - NWK ロゴは正しく表示されてます?(実機では綺麗にでなかったので改良中)<br>
  - ジョイスティックは反応しますか?(〇: 左右同時押しでポーズがかかるので, 1.01 では TOWNS PAD対応は廃止)
  - EU 版の MZ-700 は, CPUクロック, CG-ROM 及びビデオ信号規格(PAL) の違いなどから表示がおかしくなり, 三重和音の音程はやや低くなります.<br>
...
* その他小細工
  - ジョイスティックをポートに挿してなくても無限ループにならないようになってます
...
* 2021.10.11 v1.0
* 2021.10.20 v1.01 少し高速化<br>
  ステージ 7 ボス 30% 硬めに<br>
  TOWNS PAD の SELECT/RUN ボタンは誤動作しやすいので非対応に<br>
  実機でロゴが正しく表示されないバグ修正
* 2021.xx.xx v1.10 <br>
  実機でロゴが正しく表示されないバグまた修正<br>
  ゲーム モード(EASY/NORMAL/HARD/SURVIVAL/CARAVAN)を追加<br>
  ソースをかなりいじってます. steps/ フォルダは sceces/ に変更されました