...
- うっかり更新しそこなうこともあるので, 最新情報は, コマンドラインのヘルプを参照してください
 ...
 | --bars=範囲          | 指定範囲※の小節 [2, 256] のみ変換します(省略時:all, つまり2小節目以降全部)   |
...
- ファイル名は '出力Cヘッダファイル名*_*最初の小節番号*.h' になります
- 変数名は, 'sd6_*出力Cヘッダファイル名*_*最初の小節番号*' になります
```
//  Lead                    Base                    Chord0/1                          Drum
static u8 const sd6_chronos_2[] = {
    // -------- 小節2(t=0)
...
- **テンポ:** <span style="color: red; ">MIDI データでは調整できません</span>. 再生ルーチンは, 約 ♩=110 で再生します<br>再生速度を変化するには, 再生ルーチンを修正します
  - [sd6] sd6Play() の引数で調整します. sound.h を参照してください
  - [sd4] **sound-sd4.aal.c** の マクロ **TEMPO** を操作します(将来は sd4Play() の引数で調整できるようにしたい)
  - <span style="color: red; ">注意:</span> あまり再生速度を速くすると, 音符切替のオーバーヘッドが相対的に大きくなるので音の遅延が起きやすくなります
...
## 一般設定:
  - 出力音源の設定: [ファイル] → [環境設定] → [MIDI-OUT] はとりあえずポートAに, <br>デバイス=Microsoft GS Wavelength Synth<br>音源=GSm(Microsoft GS Wavelength SW Synth)
  - ♩=80～100
  - 1小節目は「セットアップ小節」で, 音色その他の初期設定に用います.<br>
    演奏データは2小節目から入力してください
## トラック関連の設定:
  - [表示] → [トラックセレクトペイン] でトラックセレクトペインを表示して, トラックの設定はこれをメインで行います
  - トラックの追加・削除は, 各トラックを右クリックして [トラックの追加], [トラックの削除]
  - トラック名, トラックに割り当てるチャンネル番号, メロディ/ドラムの種類設定は, 各トラックを右クリックして [トラックのプロパティ]
## オニオンスキンの設定:
  - オニオンスキンは, 複数のトラックの音符を透過的に表示できる機能です
  - オニオン色(トラックの色) は, 各トラックを右クリックして [オニオンスキンの色]
  - トラックを選び,  [オニオン(ニンニクの絵文字)] → [ピアノロールに指定のトラックを表示] で好きなトラックを表示できます
## 楽曲の編集
  - 音色の変更は [挿入] → [プログラム チェンジ]
  - 音符の移動はコピペで
  - Tab キーを押すと表示が変わり, 全トラックをまとめて小節単位でコピペできます
