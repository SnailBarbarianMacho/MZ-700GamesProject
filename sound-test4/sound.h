...
// ---------------------------------------------------------------- マクロ(SD4 - PWM 4音源)
// 仕様書やデータの作り方は, MIDI コンバータ sd4_midi_conv.php を参照してください
...
/** SD4 (PWM方式4重和音サウンド) で音楽を鳴らします
 * - sdcccall1 対応
 * @see SD4PLAY() ラッパーを使用してください
 */
u8 sd4play(u32 param);

/** パラメータの受け渡しを高速化するための, sd4Play() ラッパー
 * - sdcccall1 対応
 * - CPU を 100% 消費します. 鳴ってる間は 他の作業はできません
 * - ドラムデータ, 音量返還テーブル, 音楽テータ等が上位 RAM にある場合の動作は不定です
 * @param data      u16   sd4-midi-conv.php で変換した音楽データのアドレス
 ...
 * @see sd6play()
 */
#define SD4PLAY(data, tempo, can_abort) sd4play(  \
    ((((u32)(data)     ) & 0xffff) << 16) |  \
    ((((u32)(tempo)    ) &   0x7f) << 0)  |  \
