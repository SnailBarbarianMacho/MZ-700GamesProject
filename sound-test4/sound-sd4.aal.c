...
// MARK: NOTE_LEAD()
/** 音符処理 + 波長・音量変更処理(Lead)
 * @param H_wlc                 レジスタ: 波長カウンタ
 * @param L_hwl                 レジスタ: 半波長値
 * @param HL_wlchwl             レジスタ: wlc と hwl のペア
 * @param D_vol                 レジスタ: 音量
 * @param IXH_slc               レジスタ: 音長カウンタ
...
) __aal_macro __naked
{
    ...
    IXH_slc--; if (z) {                                   // 8+10
    sd4playNoteLead_pop:
        pop(HL_wlchwl);                                 // 10   注意! L_hwl=音長, H_wlc=波長

        // ---- 音長モード別の波長/音長設定
        A = L_hwl; A += A; if (nc_jr) {                 // 4+4+7/12
            A += A; IXH_slc = A; if (c_jr) {            // 4+8+7

                // ---- リピート終了
                cp(A, SD4_EXCEPT_LEN); if (z_jr) {
addr_rep_1:     HL = 0x0000; mem[HL]--; if (z_jr) {  // リピート終了
                ...
                // ---- ◢
                {
                    ...
                    A = H_wlc; mem[addr_wl_1 + 1] = A;  // 4+13 自己書換 波長
                    HL_wlchwl = addr_mod_vol; mem[addr_mod_start_1 + 1] = HL_wlchwl;  // 10+16 自己書換 波長音量変更ジャンプ先
                    HL_wlchwl = (LEAD_VOL_MAX << 8) | inc_vol;// 10
                    D_vol    = LEAD_VOL_MIN_UP;         // 7    音量
                    jp(sd4playNoteLead_initDw);         // 10 スラー無
                }
            }
            ...
            {
                SP--;
                A ^= A; mem[addr_wl_1 + 1] = A;         // 自己書換 波長 = 0(最長)
                HL_wlchwl = addr_mod_end; mem[addr_mod_start_1 + 1] = HL_wlchwl;  // 10+16 自己書換 波長音量変更ジャンプ先
                L_hwl = A;                              // 半波長 = 0(無音)
                jp(sd4playNoteLead_initEndSlur);        // スラー有
            }
        }
        A += A; IXH_slc = A; if (c_jr) {                // 4+8+7/12
            cp(A, SD4_EXCEPT_LEN); if (nc) {            // 7    A = 音長
                ...
                cp(A, SD4_EXCEPT_LEN + 4 * 4); if (c) { // 7+12
                    ...
                    H_wlc = ADDR_SD4_VTAB / 256; L_hwl = A; A = mem[HL_wlchwl];  // A = 0, 1, 3
            ...
            addr_slide_0: if (z_jr) {                   // 7 自己書換
                ...
                A = H_wlc; mem[addr_mod_wl_end_1 + 1] = A;// 4+13 自己書換 波長end
                // 現在の波長と比較してスライド方向を決める
                A = mem[addr_wl_1 + 1]; cp(A, H_wlc); A = OPCODE_INC_A; if (nc_jr) { A++; /* DEC A */} // 13+4+7+12
                mem[addr_mod_wl_inc_0 + 0] = A;         // 13  自己書換 A++/A--
                HL_wlchwl = addr_mod_wl; mem[addr_mod_start_1 + 1] = HL_wlchwl;  // 10+16 自己書換 波長音量変更ジャンプ先
                A = mem[addr_wl_1 + 1]; A >>= 1; L_hwl = A; // 13+8+4 半波長
                jp(sd4playNoteLead_initEndSlur);        // 10 スラー有
            }

            // ---- ■ スライド無
            {
                A = H_wlc; mem[addr_wl_1  + 1] = A;     // 自己書換 波長
                HL_wlchwl = addr_mod_end; mem[addr_mod_start_1 + 1] = HL_wlchwl;  // 10+16 自己書換 波長音量変更ジャンプ先
                A >>= 1;   L_hwl = A;                   // 半波長
                D_vol  = LEAD_VOL_MAX;                  // 自己書換 音量 = max
                jp(sd4playNoteLead_initEnd);            // スラー無
            }
        }
        ...
        cp(A, SD4_EXCEPT_LEN); if (nc_jr) {             // 7+7 A = 音長
            cp(A, SD4_EXCEPT_LEN + 4 * 4); if (c) {     // 7+10
                ...
                H_wlc = ADDR_SD4_VTAB / 256; L_hwl = A; A = mem[HL_wlchwl];  // A = 0, 1, 3, 7
...
sd4playNoteLead_pop0:
        ...
        // 140+140くらいになるように分割してコード量削減に努めます
        {
            ...
            A = H_wlc; mem[addr_wl_1 + 1] = A;          // 4+13 自己書換 波長
            HL_wlchwl = addr_mod_vol; mem[addr_mod_start_1 + 1] = HL_wlchwl;  // 10+16 自己書換 波長音量変更ジャンプ先
            HL_wlchwl = (LEAD_VOL_MIN_DW << 8) | dec_vol;// 10
            D_vol    = LEAD_VOL_MAX;                    // 7    自己書換 音量 = max
        }
sd4playNoteLead_initDw:
        A >>= 1; mem[sd4playNoteLead_initDw_1 + 1] = A; // 8+13 半波長
        A = H_wlc;   mem[addr_mod_vol_end_1 + 1] = A;   // 7+13 自己書換 音量end = min
        A = L_hwl;   mem[addr_mod_vol_inc_0 + 0] = A;   // 7+13 自己書換 音量変化速度(減算)
sd4playNoteLead_initDw_1: L_hwl = 0x00;                 // 7    自己書換 半波長
...
addr_mod_start_1: jp(addr_mod_end/*ジャンプ先*/);       // 10
    ...
    // ---- 音量変更◢◣
addr_mod_vol:
    A = IXH_slc; addr_mod_vol_speed_1: A &= 0x00/*音量変更速度*/; if (z_jr) {           // 8+7+7
        A = D_vol; addr_mod_vol_end_1: cp(A, 0x00/*音量end*/);                          // 4+7
        if (nz_jr) { addr_mod_vol_inc_0: D_vol++/*音量加減*/; }                         // 7+4
    }
    jp(addr_mod_end);                                   // 10

    // ---- 波長変更処理(■(スライド有))
addr_mod_wl:
    A = IXH_slc; addr_mod_wl_speed_1: A &= 0x00/*波長変更速度*/; jr_nz(addr_mod_end); { // 8+7+7
...
// MARK: NOTE_BASE()
/** 音符処理(Base)
 * @param B_wlc                 レジスタ: 波長カウンタ
 * @param C_hwl                 レジスタ: 半波長値
 * @param BC_wlwl               レジスタ: wlc と hwl のペア
 * @param E_vol                 レジスタ: 音量
 * @param IXL_slc               レジスタ: 音長カウンタ
 * @param inc_vol               命令: 音量++ (例:OPCODE_INC_E)
 * @param dec_vol               命令: 音量-- (例:OPCODE_DEC_E)
...
) __aal_macro __naked
{
    ...
    IXL_slc--; if (z_jr) {                              // 8+7
        pop(BC_wlwl);                                   // 10   注意! C_hwl=音長, reg_wlc=波長

        // ---- 音長モード別の波長/音長設定
        A = C_hwl; A += A; if (nc_jr) {                 // 4+4+7/12
            A += A; IXL_slc = A; if (c_jr) {            // 4+8+4

                // ---- ◢
                A = B_wlc; mem[addr_wl_1 + 1] = A;      // 4+13 自己書換 波長
                A >>= 1;     C_hwl = A;                 // 8+4  半波長
                E_vol = 0;                              // 7    音量 = 0
            ...
            A += A; IXL_slc = A;                        // 4+8  音長 cf:0/1 = ◣■
            A = B_wlc; mem[addr_wl_1 + 1] = A;          // 4+13 自己書換 波長
            E_vol = BASE_VOL_MAX;                       // 音量
            ...
            if (c_jr) {                                 // 12
                ...
                // ---- ◣
                rra(A);      C_hwl = A;                 // 4+4 半波長. cf=0 なので srl でなくても OK
...
) __aal_macro __naked
{
    ...
    A = E_vol; addr_mod_vol_end_1: cp(A, 0x00/*音量end*/);  // 4+7      自己書換
    if (nz_jr) { addr_mod_vol_inc_0: E_vol++/*音量加減*/; } // 7+4      自己書換
...
/** 音符処理 (Chord2)
 * - コードは音長と音量が兼用です
 * @param B_wl0                 レジスタ: 波長0 / 波長カウンタ
 * @param C_wl1                 レジスタ: 波長1 / 半波長
 * @param BC_wl01               レジスタ: wl0 と wl1 のペア
 * @param D_vol                 レジスタ: 音量
 * @param IYH_slc               レジスタ: 音長カウンタ
 * @param inc_vol               命令: 音量++ (例:OPCODE_INC_D)
 * @param dec_vol               命令: 音量-- (例:OPCODE_DEC_D)
...
) __aal_macro __naked
{
    ...
    IYH_slc--; if (z) {                                 // 8+10
        ...
        // ---- 音長モード別の波長/音長設定
        A += A; if (nc_jr) {                            // 4+7/12
            A += A; IYH_slc = A; if (c_jr) {            // 4+8+7

                // ---- ◢
                // chord2 処理時間のワーストケースは 205(◢).
                // 120+120くらいになるように分割してコード量削減に努めます
                pop(BC_wl01);                           // 10
                A = B_wl0; mem[addr_mod_wl0_1 + 1] = A; // 4+13 自己書換 波長0
                A = C_wl1; mem[addr_mod_wl1_1 + 1] = A; // 4+13 自己書換 波長1
                D_vol = 0;                              // 4 音量 = 0
                BC_wl01 = (CHORD_VOL_MAX << 8) | inc_vol;// 10 音量end|音量変化速度(加算)
                jp(sd4playNoteChord2_initDw);           // 10
            }

            // ---- 休符 ... 音量設定不要
            A ^= A; mem[addr_beep_wl_1 + 1] = A;        // 自己書換 波長0 = 0(最長)
            C_wl1 = A;                                  // 半波長 = 0(無音)
            A = OPCODE_JR;                              // 波長処理無(音量処理も無)
            jp(sd4playNoteChord2_initEnd);
        }
        A += A; IYH_slc = A;                            // 4+8  音長 cf:0/1 = ◣■
        pop(BC_wl01);                                   // 10
        A = B_wl0; mem[addr_mod_wl0_1 + 1] = A;         // 4+13 自己書換 波長0
        A = C_wl1; mem[addr_mod_wl1_1 + 1] = A;         // 4+13 自己書換 波長1
        D_vol = CHORD_VOL_MAX;                          // 7    音量
        if (c) {                                        // 10
            ...
            // ---- ◣
            BC_wl01 = (CHORD_VOL_MIN_DW << 8) | dec_vol;// 10 音量end|音量変化速度(減算)
sd4playNoteChord2_initDw:
            A = mem[VRAM_TEXT]; ex(AF, AF); mem[MMIO_8253_CT0] = A; ex(AF, AF);// /VBLK=L (ブランク中) になるまで待ち, 音を鳴らす
            A = B_wl0; mem[addr_mod_vol_end_1   + 1] = A;// 4+13 自己書換 音量end
            A = C_wl1; mem[addr_mod_vol_inc_0   + 0] = A;// 4+13 自己書換 音量変化速度
...
) __aal_macro __naked
{
    ...
    // ---- 波長変更(アルペジオ)◢■◣
    A = IYH_slc; A &= 2; addr_mod_wl0_1: A = 0x00/*波長0*/;     // 8+7+7 cf=0   自己書換
    if (z_jr) {          addr_mod_wl1_1: A = 0x00/*波長1*/;}    // 7+7          自己書換
    mem[addr_beep_wl_1 + 1] = A; rra(A); C_hwl = A;             // 13+4+4 rra時cf=0

    // ---- 音量変更◢◣
    A = IYH_slc; A &= 1;
    addr_mod_vol_start_0: jr_nz(sd4playModChord_end)/*音量変更有無*/;   // 8+7+7 自己書換
    A = D_vol; addr_mod_vol_end_1: cp(A, 0x00/*音量end*/);      // 4+7   自己書換
    if (nz_jr) { addr_mod_vol_inc_0: D_vol++/*音量加減*/; }     // 7+4   自己書換
...
// MARK: NOTE_DRUM()
/** 音譜処理(Drum)
 * @param IYL_sl            レジスタ: 音長カウンタ
 * @param L_tmp             レジスタ: テンポラリ
    ...
    int addr_drum_ct_1, int addr_drum_nr_1, int addr_drum_add_0) __aal_macro __naked
{
    ...
    IYL_sl--; if (z) {                                  // 8+10
        SP--; pop(AF);                                  // 6+10
        L_tmp = A; A &= 0xfc; IYL_sl = A;               // 4+7+8 音長
        A ^= L_tmp;                                     // 4     reg_tmp と A を xor すると, ドラム番号(0-3)だけ残る
...
sd4PlayModDrum_ct_1: A = 0x00/* ドラム カウンタ */; A |= A; if (nz_jr) { // 7+4+7
        ...
        if (nz_jr) {
            ...
            A ^= A;             mem[addr_drum_add_0 + 0] = A;   // 12+7+13 自己書換 ドラム音を鳴らす
        }
        // ワーストケース=72
    }
sd4PlayModDrum_end:
 ...
 */
void SD4PLAY_BEEPER_WAVE_R(int H_wlc, int L_hwl, int D_vol, int addr_wl_1) __aal_macro __naked
{
    AAL_DEF_VARS;

    // 波長処理
    H_wlc--; if (z) {                                   // 4+10
        addr_wl_1: H_wlc = 0x00;/* 波長を設定したらその場は音量処理は不要 */
    } else {
        // 音量処理
        A = H_wlc; cp(A, L_hwl);                        // 4+4
        if (c_jr) { ex(AF, AF); A += D_vol; ex(AF, AF); }// 7+4+4+4
    }
    // ワースト ケース=41
 ...
 * @param B_wlc         レジスタ: 波長カウンタ (Bで固定)
 * @param C_hwl         レジスタ: 半波長値
 * @param E_vol         レジスタ: 音量
 * @param addr_wl_1     アドレス(自己書換): 波長をセットする
 */
void SD4PLAY_BEEPER_WAVE_BR(int B_wl, int C_hwl, int E_vol, int addr_wl_1) __aal_macro __naked
{
    AAL_DEF_VARS;
    ...
    // 波長処理
    djnz(B_wl, sd4play_beeperWaveBr); {                 // 13
        addr_wl_1: B_wl = 0x00; jr(sd4play_beeperWaveBrEnd);/* 波長を設定したらその場は音量処理は不要 */
    }
    sd4play_beeperWaveBr: {
        // 音量処理. vol < 2 ならば, 波長カウンタの値に依らず cf が立たない
        A = B_wl; cp(A, C_hwl);                         // 4+4
        if (c_jr) { ex(AF, AF); A += E_vol; ex(AF, AF); }// 7+4+4+4
    }
 ...
 */
void SD4PLAY_BEEPER_WAVE_B(int B_wlc, int C_hwl, int addr_vol_1, int addr_wl_1) __aal_macro __naked
{
    AAL_DEF_VARS;
    AAL_LOCAL(sd4play_beeperWaveBr, sd4play_beeperWaveBrEnd);
    extern sd4play_beeperWaveBr, sd4play_beeperWaveBrEnd;
    ...
    // 波長処理
    djnz(B_wlc, sd4play_beeperWaveBr); {                    // 13
        addr_wl_1: B_wlc = 0x00; jr(sd4play_beeperWaveBrEnd);/* 波長を設定したらその場は音量処理は不要 */
    }
    sd4play_beeperWaveBr: {
        // 音量処理. vol < 2 ならば, 波長カウンタの値に依らず cf が立たない
        A = B_wlc; cp(A, C_hwl);                     // 4+4
 ...
 * @param H_wav_h           レジスタ: ドラム波形テーブル
 * @param L_wav_l           レジスタ: ドラム波形テーブル
 * @param HL_wav_hl         レジスタ: H_wav_h と L_wav_l のペア
 * @param E_ct              レジスタ: カウンタ
 * @param addr_drum_ct_1    アドレス: 自己書換 カウンタ変更命令
 * @param addr_drum_nr_1    アドレス: 自己書換 ドラム番号
 * @param addr_drum_add_0   アドレス: 自己書換 ドラム音を鳴らす
 */
...
{
    AAL_DEF_VARS;
    L_wav_l = E_ct; addr_drum_ct_1: set(0, L_wav_l);              // 4+8  自己書換
    addr_drum_nr_1: H_wav_h = (ADDR_SD4_DRUM - 1) / 256 + 1/*ドラム番号*/; // 7    自己書換
...
// MARK: play()
u8 sd4play(u32 param) __aal
{                                                       // HL = data, D = 中断可能フラグ, E = テンポ[0, 127]
    AAL_DEF_VARS;
...
extern tmp;
    ...
    bit(0, D); if (z_jr) {                              // D = 中断許可フラグ
        A = 0xfa; mem[MMIO_8255_PORTA] = A;
    }

    // ---- テンポ
    A = E; mem[sd4play_tempo_1 + 1] = A;                // E = テンポ[0, 127]
    ...
    // ---- 同期取ります
    A ^= A; HL = MMIO_8255_PORTC;
sd4play_vblk1:  A |= mem[HL]; jp_p(sd4play_vblk1);      // /VBLK=H (表示中) になるまで待つ
