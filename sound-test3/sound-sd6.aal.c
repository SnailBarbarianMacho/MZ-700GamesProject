...
// 特別な命令に使われる音長. この音長の音符は使えません(4の倍数, makefile で定義)
 ...
 * @param H_tmp                 レジスタ: テンポラリとして H を使用します
 * @param L_tmp                 レジスタ: テンポラリとして L を使用します
 * @param HL_tmp                レジスタ: テンポラリとして HL を使用します
...
) __aal_macro __naked
{
    ...
    IXH_slc--; if (z) {                             // z になる割合は 1/4～1/256
    sd6playNoteLead_pop:
        pop(HL_tmp);                                    // H = 波長, L = 音長

        // ---- 音長モード別の波長/音長設定
        A = L_tmp; A += A; if (nc) {
            A += A; IXH_slc = A; if (nc_jr) {
                // ---- リピート
                ...
                jp(sd6playNoteLead_initEndSlur);        // スラー有
            }

            // ---- リピート終了
            cp(A, SD6_EXCEPT_LEN); if (z_jr) {
addr_rep_1:     HL = 0x0000; mem[HL]--; if (z_jr) {     // リピート終了 自己書換
            ...
            // ---- ◢
            {
                A = H_tmp; mem[addr_wl_1 + 1] = A;      // 自己書換 波長
                H_tmp = ADDR_SD6_TAB / 256; L_tmp = A; A = mem[HL_tmp]; // 音量 = 波長->パルス幅変換テーブル
                ...
                jp(sd6playNoteLead_initEnd);            // スラー無
            }
        }
        A += A; IXH_slc = A; if (nc_jr) {
            // ---- エンベロープ(音量)速度設定
...
addr_slide_0: if (z_jr) {                               // 自己書換 jr/ld A,n = スライド無/有
            A = H_tmp; mem[sd6playNoteLead_modWlEnd_1 + 1] = A; // 自己書換 波長end
            // 現在の波長と比較してスライド方向を決める
            A = mem[addr_wl_1 + 1]; cp(A, H_tmp); A = OPCODE_INC_A; if (nc_jr) { A++; /* DEC A */}
...
sd6playNoteLead_initEnd:
        B_wlc = 1;                                      // スラー無 (波長カウンタ リセット)
#if BEEPER1_SYNC
        A = OPCODE_LD_A_N; mem[addr_beeper1_sync_0] = A;// Beeper1 同期
#endif

sd6playNoteLead_initEndSlur:
        mem[sd6playNoteLead_modBranch_1 + 1] = HL;      // 自己書換 波長音量変更ジャンプ先
    }

    // -------- 波長・音量変更処理
sd6playNoteLead_modBranch_1: jp(sd6playNoteLead_modEnd/*ジャンプ先*/);

    // ---- 波長変更処理(■(スライド有))
sd6playNoteLead_modWl:
    A = IXH_slc; sd6playNoteLead_modWlSpeed_1: A &= 0x00/*波長変更速度*/; jr_nz(sd6playNoteLead_modEnd); {
...
sd6playNoteLead_modVol:
    A = IXH_slc; addr_mod_vol_speed_1: A &= 0x00/*音量変更速度*/; if (z) {
    ...
    IXH_slc--; if (z) {                                 // z になる割合は 1/4～1/256
        ...
        A = L_tmp; A += A; if (nc) {
            A += A; IXH_slc = A; if (nc_jr) {
                ...
                A = OPCODE_JR;                          // 分岐でジャンプ
                jp(sd6playNoteBase_initEnd);            // 音量 = 0
            }
            {
                // ---- ◢
                A = H_tmp; mem[addr_wl_1 + 1] = A;      // 自己書換 波長
...
sd6playNoteBase_modStart:
    ...
    // ---- 音量変更◢◣
    A = IXH_slc; A &= BASE_VOL_SPEED_R; if (z) {
 ...
 * - コードは音長と音量が兼用です
 * @param B_wlc0        レジスタ: 波長カウンタ0
 * @param C_wlc1        レジスタ: 波長カウンタ1
 * @param BC_wlc01      レジスタ: (B_wlc0 << 8) | C_wlc1
 * @param IXH_slc       レジスタ: 音長カウンタ
 * @param H_tmp         レジスタ: テンポラリとして H を使用します
 * @param L_tmp         レジスタ: テンポラリとして L を使用します
 * @param HL_tmp        レジスタ: テンポラリとして HL を使用します
 * @param D_vol01       レジスタ:           Beeper 音量0/1兼用
...
) __aal_macro __naked
{
    ...
    IXH_slc--; if (z) {                                 // z になる割合は 1/4～1/256
        ...
        A += A; if (nc) {
            A += A; IXH_slc = A; if (nc_jr) {
                // ---- 休符
                A ^= A; mem[addr_wl0_1 + 1] = A;        // 自己書換 波長0 = 0(最長)
                mem[        addr_wl1_1 + 1] = A;        // 自己書換 波長1 = 0(最長)

                D_vol01 = A;                            //          音量0/1兼用 = 0(無音)
            ...
            }
            {
                ...
                // ---- ◢
                A = C_wlc1; mem[addr_wl1_1 + 1] = A;    // 自己書換 波長1
                A = B_wlc0; mem[addr_wl0_1 + 1] = A;    // 自己書換 波長0
...
sd6playNoteChord2_modStart_0: jr(sd6playNoteChord2_modEnd/*ジャンプ先*/);
    ...
    A = IXH_slc; A &= CHORD2_VOL_SPEED_R; if (z) {
        // ---- 音量変更◢◣0/1兼用
 ...
 */
void SD6PLAY_NOTE_DRUM(int IYL_sl, int L_tmp, int addr_drum_nr_1) __aal_macro __naked
{
    AAL_DEF_VARS;

    A = 0x46;                                           // ドラム OFF
    IYL_sl--; if (z) {                                  // z になる割合は 1/8～1/256
        SP--; pop(AF);
        L_tmp = A; A &= 0xfc; IYL_sl = A;               // 音長
        A ^= L_tmp;                                     // L_tmp と A を xor すると, ドラム番号(0-3)だけ残る
 ...
 * @param C_wl          レジスタ: 波長カウンタ
 * @param E_pulse       レジスタ: 8255パルス
 * @param addr_vol_1    アドレス: 音量をセットする(8255パルス幅を決める)
 * @param addr_wl_1     アドレス: 波長をセットする
 */
void SD6PLAY_BEEPER_WAVE(int C_wl, int E_pulse, int addr_vol, int addr_wl_1) __aal_macro __naked
{
    AAL_DEF_VARS;

    // 波長処理
    C_wl--; if (z) {
addr_wl_1: C_wl = 0x00;                             // 波長を設定したらその場は音量処理は不要
    } else {
        // 音量処理 vol < 2 ならば, 波長カウンタの値に依らず cf が立たない
        A = C_wl;
addr_vol_1: cp(A, 0x02/*音量*/); if (c_jr) { E_pulse = PULSE_H; }
    }
 ...
 * @param C_wl      レジスタ: 波長カウンタ
 * @param E_pulse   レジスタ: 8255パルス値
 * @param H_vol     レジスタ: 音量(8255パルス幅)
 * @param addr_wl_1 アドレス: 波長をセットする
 */
void SD6PLAY_BEEPER_WAVE_R(int C_wl, int E_pulse, int H_vol, int addr_wl_1) __aal_macro __naked
{
    AAL_DEF_VARS;
...
addr_wl_1: C_wl = 0x00;/* 波長を設定したらその場は音量処理は不要 */
        ...
        // 音量処理. vol < 2 ならば, 波長カウンタの値に依らず cf が立たない
        A = C_wl; cp(A, H_vol); if (c_jr) { E_pulse = PULSE_H; }
    }
 ...
 * @param B_wl      レジスタ: 波長カウンタ=B
 * @param E_pulse   レジスタ: 8255パルス値
 * @param H_vol     レジスタ: 音量(8255パルス幅)
 * @param addr_wl_1     アドレス: 波長をセットする
 */
void SD6PLAY_BEEPER_WAVE_BR(int B_wl, int E_pulse, int H_vol, int addr_wl_1) __aal_macro __naked
{
    AAL_DEF_VARS;
...
addr_wl_1: B_wl = 0x00; jr(sd6play_beeperWaveBrEnd);/* 波長を設定したらその場は音量処理は不要 */
    }
sd6play_beeperWaveBr: {
        // 音量処理. vol < 2 ならば, 波長カウンタの値に依らず cf が立たない
        A = B_wl; cp(A, H_vol); if (c_jr) { E_pulse = PULSE_H; }
    }
 ...
 * @param HL_wav            レジスタ: ドラム波形テーブル
 * @param E_pulse           レジスタ: 8255パルス値
 * @param addr_drum_on      アドレス: ドラム処理を飛ばす命令
 * @param addr_drum_nr_1    アドレス: ドラム波形テーブルのビット命令
 */
void SD6PLAY_BEEPER_DRUM(int HL_wav, int E_pulse, int addr_drum_nr_1) __aal_macro __naked
{
    AAL_DEF_VARS;

addr_drum_nr_1: bit(0/*ドラム波形テーブル ビット位置*/, mem[HL_wav]); // 自己書換
    if (nz_jr) { E_pulse = PULSE_H; }// 12+7+7 or 12+12
...
extern tmp;
    ...
    //   A' I           未使用
    mem[sd6play_restoreSP_1 + 1] = SP; SP = HL;           // SP 保存
    A ^= A;
    C = BEEPER1_WL_CT_VAL; mem[sd6play_beeper1_wl_1 + 1] = A;// Beeper1 の初期化 音長 = 0
    A++;
    IXH = A; IXL = A; IYL = A; IYH = A;                 // Lead, Base, Chord2, Drum 音長カウンタ = 1
    A = 7;         mem[sd6play_modVolSpeed0_1 + 1] = A; // Lead 音量変化速度
...
sd6play_loop:
    ...
    SD6PLAY_NOTE_LEAD(B,  IXH, H, L, HL,
        sd6play_rep_1,
        sd6play_setVols + 1, sd6play_beeper0_wl_1, sd6play_slide0, sd6play_modVolSpeed0_1, sd6play_beeper1_sync, sd6play_end);
...
sd6play_beeperLoop:
        ...
        SD6PLAY_BEEPER_WAVE_BR(    B,  E,    H,  sd6play_beeper0_wl_1);// 33 Lead
        SD6PLAY_BEEPER_WAVE_R(     C,  E,    H,  sd6play_beeper1_wl_1);// 36 Lead(echo)
