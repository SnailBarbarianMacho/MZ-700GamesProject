...
// ---------------------------------------------------------------- マクロ
#define CT_8253_CH1 262 // 8253のカウンタ. 1カウンタ1ライン. 262ラインで1フレーム
...
VRAM_VBLK_SYNC1:
    and     (HL)                    // /VBLK = L になるまで待つ
    jp      m,  VRAM_VBLK_SYNC1     // '1' ならばループ
VRAM_VBLK_SYNC2:
    or      (HL)                    // /VBLK = H になるまで待つ
    jp      p,  VRAM_VBLK_SYNC2     // '0' ならばループ

    // 228(T state)x198(VLines)=45144 だけ待ってからカウント開始
    ld      B, #13                  // 7
VRAM_VBLK_SYNC3:
    ld      A, #246                 // 7
VRAM_VBLK_SYNC4:
    dec     A                       // 4
    jp      nz, VRAM_VBLK_SYNC4     // 10/10
    djnz    B,  VRAM_VBLK_SYNC3     // 13/8
    // 小計 7+(7+(4+10)*247+13)*13-5 = 45216
    // 小計 7+(7+(4+10)*246+13)*13-5 = 45034
    // 小計 7+(7+(4+10)*245+13)*13-5 = 44852

    // ch1 カウンタを設定してカウント開始!
    ld      L,    #(MMIO_8253_CTRL & 0xff)
    ld      (HL), #MMIO_8253_CTRL_RL_LH_MASK | MMIO_8253_CTRL_CH1_MASK | MMIO_8253_CTRL_MODE2_MASK
    ld      L,    #(MMIO_8253_CH1 & 0xff)
    ld      (HL), #(CT_8253_CH1 & 0xff)
    ld      (HL), #(CT_8253_CH1 >> 8)    // カウント開始
